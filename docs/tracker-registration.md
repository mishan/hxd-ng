...
Hotline wire; the password is converted at the same edge. All three are
validated against the one-byte length limit rather than silently truncated.
...
compatible prefix, but a conforming v3 tracker gives the HMAC precedence.

...
**Begun: threaded news with plain bodies, on the ng wire, and search.**
The domain (`hxd-core::news`, stage W1), the SQLite store (W2), full-text
search over FTS5 with its closed query grammar and `hxd news-reindex`
(W4, schema version 4), and the ng requests, events and login block that
need nothing from markdown or attachments (the core of W6) have landed,
with conformance suites both stores pass and hx-ng's News view as the
first client. Next, in the order `docs/news.md` §16 allows: markdown
(W3) and attachments (W5), then subscriptions and notifications (W7),

//! What a push endpoint may look like (`docs/webpush-gateway.md` §6).
//!
...
//! there has ever been. The check runs twice and in two places: the URL's
//! shape is checked here, at registration, so a client is told no while
//! it is still asking — and the address it resolves to is checked by the
//! gateway before every send, because DNS moves and a name that answered
//! publicly last week can answer `127.0.0.1` today. The half that needs
//! a resolver is not the domain's business; this half is, because
//! refusing a registration is.
...
/// and answer where it would connect.
///
...
pub fn shape(url: &str) -> Result<Target, Refused> {
...
/// What a registration checks: the shape, and — unless the operator has
/// lifted the address check with `[push] allow_private_endpoints` — the
/// address itself where the host is a literal. A literal can be judged
/// here and now; a name cannot, and is the gateway's to resolve and
/// check before every send.
pub fn registrable(url: &str, allow_private: bool) -> Result<Target, Refused> {
    let target = shape(url)?;
    if !allow_private {
        if let Ok(addr) = target.host.parse::<IpAddr>() {
            if !is_public(addr) {
                return Err(Refused::NotGloballyRoutable);
            }
        }
    }
    Ok(target)
...
mod tests {
    ...
    fn a_push_endpoint_is_an_https_url_with_a_host() {
        assert_eq!(
            shape("https://push.example.net/v/abc"),
            Ok(Target {
        ...
        assert_eq!(
            shape("https://push.example.net:8443/v/abc").unwrap().port,
            8443
        );
        assert_eq!(
            shape("https://[2606:4700::1]:8443/v").unwrap(),
            Target {
                host: "2606:4700::1".into(),
                port: 8443
            }
        );
        assert_eq!(shape("http://push.example.net/v"), Err(Refused::NotHttps));
        assert_eq!(shape("/v/abc"), Err(Refused::NotHttps));
        assert_eq!(shape("https:///v/abc"), Err(Refused::NotHttps));
        assert_eq!(
            shape("https://user:pw@push.example.net/v"),
            Err(Refused::HasUserinfo)
        );
        assert_eq!(
            shape("https://push.example.net:nope/v"),
            Err(Refused::NotHttps)
        );
        assert_eq!(
            shape("https://push.example.net/v#frag"),
            Err(Refused::HasFragment)
        );
    }

    #[test]
    fn a_literal_address_is_checked_where_it_stands() {
        // A name needs a resolver and is the gateway's to check before
        // every send; a literal needs nothing, so a registration naming
        // one is refused while the client is still asking.
        let reg = |url| registrable(url, false);
        assert_eq!(
            reg("https://127.0.0.1/v/1"),
            Err(Refused::NotGloballyRoutable)
        );
        assert_eq!(reg("https://[::1]/v/1"), Err(Refused::NotGloballyRoutable));
        assert_eq!(
            reg("https://[::ffff:10.0.0.1]/v/1"),
            Err(Refused::NotGloballyRoutable),
            "including a v4 address wearing a v6 hat"
        );
        assert_eq!(
            reg("https://[64:ff9b::a00:1]/v/1"),
            Err(Refused::NotGloballyRoutable),
            "and one that NAT64 would carry"
        );
        assert!(reg("https://1.1.1.1/v/1").is_ok());
    }

    #[test]
    fn the_operators_private_push_service_is_theirs_to_allow() {
        assert_eq!(
            registrable("https://10.0.0.5:8443/v/1", true).unwrap(),
            Target {
                host: "10.0.0.5".into(),
                port: 8443
            },
            "allow_private_endpoints lifts the literal check too"
        );
        assert_eq!(
            registrable("http://10.0.0.5/v/1", true),
            Err(Refused::NotHttps),
            "and nothing else"
        );
    }
